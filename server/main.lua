...
ESX.RegisterServerCallback("jav_invetoryhud:getPlayerInventory",
...
RegisterServerEvent("jav_invetoryhud:tradePlayerItem")
AddEventHandler("jav_invetoryhud:tradePlayerItem", function(from, target, type, itemName, itemCount)
    local _source = from
...
RegisterCommand("openinventory", function(source, args, rawCommand)
    if IsPlayerAceAllowed(source, "inventory.openinventory") then
        ...
        if targetXPlayer ~= nil then
            TriggerClientEvent("jav_invetoryhud:openPlayerInventory", source, target, targetXPlayer.name)
        else
