...
window.addEventListener("message", function (event) {
    if (event.data.action == "display") {
        ...
        $(".nearbyPlayerButton").click(function () {
            ...
            $.post("http://jav_invetoryhud/GiveItem", JSON.stringify({
                player: player,
...
function closeInventory() {
    $.post("http://jav_invetoryhud/NUIFocusOff", JSON.stringify({}));
}
	...
	$('#itemFast-1').droppable({
        drop: function (event, ui) {
            ...
            if (type === "normal" && (itemInventory === "main" || itemInventory === "fast") && itemData.type === "item_weapon") {
                ...
                $.post("http://jav_invetoryhud/PutIntoFast", JSON.stringify({
                    item: itemData,
                    slot : 1
                }));
            }
...
$(document).ready(function () {
    ...
    $('#use').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.usable) {
                ...
                $.post("http://jav_invetoryhud/UseItem", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#give').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("http://jav_invetoryhud/GetNearPlayers", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#drop').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("http://jav_invetoryhud/DropItem", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://jav_invetoryhud/TakeFromTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://jav_invetoryhud/TakeFromProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val()),
					owner : ownerHouse
                }));
                ...
                disableInventory(500);
                $.post("http://jav_invetoryhud/TakeFromPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }else if (type === "normal" && itemInventory === "fast") {
                disableInventory(500);
                $.post("http://jav_invetoryhud/TakeFromFast", JSON.stringify({
                    item: itemData
                }));
            } else if (type === "glovebox" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://jav_invetoryhud/TakeFromGlovebox", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                })); 
            } else if (type === "shop" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://jav_invetoryhud/TakeFromShop", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "disc-property" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://jav_invetoryhud/TakeFromDiscProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://jav_invetoryhud/PutIntoTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://jav_invetoryhud/PutIntoProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val()),
					owner : ownerHouse
                }));
                ...
                disableInventory(500);
                $.post("http://jav_invetoryhud/PutIntoGlovebox", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "disc-property" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://jav_invetoryhud/PutIntoDiscProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://jav_invetoryhud/PutIntoPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
